...
The existing event listener calls `openChat()` then dispatches a `sendMessage()` after 300ms. If `openChat()` sets `view = "capture"` (first visit), `sendMessage()` must be blocked while the capture screen is visible. The implementation must guard `sendMessage()` with `if (view !== "capture")` so the queued message fires only after the user submits or skips. The pre-filled message will be lost in this case — the user lands on the capture screen and types their own message after. This is acceptable: the package card "Get Started" flow is a convenience, not a hard requirement, and the contact capture takes priority on first visit.
